...
Expe.colscreen=[80 80 80];
Expe.ElSampRate=250;%the eyelink desired samprate (250 or 500)
Expe.FixationPoint=[Expe.DispSize(3)/2 80];
Expe.Date=datestr(now);
Expe.ImPath='C:\copy\Expe075_FaceVoiceAssociation\Stimuli\Images\';
Expe.SdPath='C:\copy\Expe075_FaceVoiceAssociation\Stimuli\Sounds\';
Expe.AnimPath='C:\copy\Expe075_FaceVoiceAssociation\Stimuli\Anims\';
Expe.NumPres=6;%number of presentation
Expe.PresDur=10;%duration of stimulus either for presentation or for looking time
Expe.wRect = get(0,'screensize'); % what does it mean
...
%% GetAnimList
ListAnim=dir(strcat(Expe.AnimPath,'*.avi'));
...
for j = 1: length(ListSd)
    [Y,FS,NBITS]=wavread(strcat(Expe.SdPath,char(ListSd{Expe.SdOrder(j)}))); % what does Y mean exactly?
...
AOI=zeros(Expe.wRect(4),Expe.wRect(3));% what does it mean?
AOI(Expe.AOI(1,2):Expe.AOI(1,4),Expe.AOI(1,1):Expe.AOI(1,3))=1;% what does it mean?
...
Expe.ScreenNumber=max(Screen('Screens'));
    % Open a graphics window on the main screen
    % using the PsychToolbox's Screen function (type Screen('OpenWindow?') to
    % have detail on the function
    %[winptr, wRect] = Screen('OpenWindow', ScreenNumber, 80)
    [winptr, Expe.wRect] = Screen('OpenWindow', Expe.ScreenNumber,Expe.colscreen(1,:), [], [], [], 0, [],...
        kPsychNeedFastBackingStore);  
    Screen('BlendFunction', winptr, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    % psychtoolbox commands to remove the cursor and move to the command window
    commandwindow;
    HideCursor;
...
%             end
        ...
        case 'Display'
            ...
            if FPass==1;
                ...
                Screen('DrawTexture', winptr, tex1, [0 0 size(Stim(i).Image,2) size(Stim(i).Image,1)],...
                [Expe.AOI(1,:)])
        ...
        case 'Sound'
            ...
            if FPass==1;
                ...
                CurrTime=GetSecs;
                TrialTime=CurrTime;
                [LookRes winptr]=ShowMovies( Expe.AnimPath,char(ListAnim(Expe.AnimOrder(trial))),[],99,1,0,winptr,[],el,2,Expe.PresDur,0);
            Results(trial,3)=LookRes(5);
